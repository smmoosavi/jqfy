...
function JQfy() {
    ...
    function defineGetter(cg, loader) {
        var getter = 'templates._GETTER = ' + (function (data, key) {
        ...
        var escape = 'templates._ESCAPE = ' + (function (string) {
                var htmlEscapes = {
                    '&': '&amp;',
                    '<': '&lt;',
                    '>': '&gt;',
        ...
        try {
            getter = loader('/_GETTER.js');
        } catch (e) {
        }
        try {
            escape = loader('/_ESCAPE.js');
        } catch (e) {
        }
        cg.raw();
        cg.raw(getter);
    ...
    function extractVariables(text, useGetter) {
        if (useGetter === undefined) {
            useGetter = false;
        }
        var pat = /\{\{([^\{\}]+)\}\}/gm;
        var match;
        ...
        while (true) {
            ...
            if (useGetter) {
                out = out + util.format(' + templates._GETTER(data , %s) + ', JSON.stringify(variable));
            } else {
    ...
    function writeCreateTextNode(cg, parent, text, opts) {
        ...
        var value = extractVariables(text, opts._useGetter);
    ...
    function writeCreateComment(cg, parent, text, opts) {
        ...
        var comment = extractVariables(text, opts._useGetter);
        if (opts._useShortcutFunctions) {
    ...
    jqfy.compile = function (html, opts) {
        ...
        function generate(node, parentCtx) {
            ...
            if (node.type == 'tag') {
                ...
                cg.incIndent();
                if (node.attribs.id) {
                    cg.rawFormat('.attr(%s, %s)', JSON.stringify('id'), extractVariables(node.attribs.id, opts._useGetter));
                }

                if (node.attribs['class']) {
                    cg.rawFormat('.addClass(%s)', extractVariables(node.attribs.class, opts._useGetter));
                }
                ...
                Object.keys(node.attribs).forEach(function (a) {
                    ...
                    cg.rawFormat('.attr(%s, %s)', extractVariables(a, opts._useGetter), extractVariables(v, opts._useGetter));
                });
            ...
            if (node.type == 'text') {
                ...
                if (text) {
                    writeCreateTextNode(cg, parentCtx.parent, text, opts);
                }
            }
            if (node.type == 'comment') {
                if (opts.comment) {
                    writeCreateComment(cg, parentCtx.parent, node.data, opts);
                }
            }
            if (node.type == 'script') {
    ...
    jqfy.getCode = function (name, opts) {
        ...
        memory.forEach(function (item) {
            ...
            compileOpts._useGetter = opts.useGetter;
        ...
        defineGetter(output, opts._loader);
